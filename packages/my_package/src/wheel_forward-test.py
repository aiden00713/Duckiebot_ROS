...
W_LEFT = 0.4 * (2 * math.pi)  # 表示每秒左輪轉 0.8 圈
W_RIGHT = 0.8 * (2 * math.pi)  # 表示每秒右輪轉 0.5 圈
...
class WheelControlNode(DTROS):
    ...
    def __init__(self, node_name):
        ...
        wheels_topic = f"/{vehicle_name}/car_cmd_switch_node/cmd"
    ...
    def run(self):
        ...
        start_time = rospy.Time.now()  # 紀錄開始時間
        ...
        while not rospy.is_shutdown():
            ...
            if elapsed_time >= 3.0:
                print("---Execution finished after 3 seconds.---")
                break
